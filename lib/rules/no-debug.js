...
function isRenderVariableDeclarator(node, renderFunctions) {
  const isRenderFunction = callNode =>
    ['render', ...renderFunctions].some(name => name === callNode.callee.name);

  if (node.init) {
    if (node.init.type === 'AwaitExpression') {
      return node.init.argument && isRenderFunction(node.init.argument);
    } else {
      return node.init.callee && isRenderFunction(node.init);
    }
  }
